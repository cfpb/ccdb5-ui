...
import { focusChanged } from '../../../reducers/trends/trends-slice';
import { rowCollapsed, rowExpanded } from '../../../reducers/view/view-slice';
    ...
    const wrapText = (text, width, viewMore) => {
      ...
      text.each(function () {
        ...
        const words = innerText.text().split(/\s+/).toReversed(),
          ...
          dy = Number.parseFloat(innerText.attr('dy') || 0);
        ...
        if (wrapCount) {
          ...
          const newHeight = Number.parseFloat(oldHeight) + wrapCount * 12;
