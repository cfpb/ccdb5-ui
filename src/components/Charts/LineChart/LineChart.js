...
export const LineChart = () => {
  const trendsResultsDateRangeLine = useSelector(
    selectTrendsResultsDateRangeLine,
  );
  ...
  useEffect(() => {
    //const processData = cloneDeep(trendsResultsDateRangeLine);
    //pruneIncompleteLineInterval(processData, dateRange, queryDateInterval);

    setHasChart(
      Boolean(data.dataByTopic && data.dataByTopic[0].dates.length > 1),
    );
    // hasChart = Boolean(processData.dataByTopic && processData.dataByTopic[0].dates.length > 1)
    ...
    const redrawChart = () => {
      ...
      const colorScheme = data.dataByTopic.map(
      ...
      container.datum(cloneDeep(data)).call(lineChart);
      ...
      if (queryLens !== 'Overview') {
        ...
        const item = getLastLineDate(data, config);
    ...
    hashObject(trendsResultsDateRangeLine),
    trendsTooltip,
    trendsColorMap,
    viewWidth,
    viewIsPrintMode,
